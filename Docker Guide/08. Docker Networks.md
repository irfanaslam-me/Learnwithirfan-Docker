...
graph TD
    subgraph HOST["Host Machine"]
        subgraph BRIDGE["bridge — default, containers talk by IP"]
            C1["container1\n172.17.0.2"]
            C2["container2\n172.17.0.3"]
            C3["container3\n172.17.0.4"]
            GW["docker0 gateway\n172.17.0.1"]
        ...
        subgraph CUSTOM["custom bridge — containers talk by NAME"]
            WEB["web container\nping db ✅"]
            DB["db container\nping web ✅"]
            WEB <-->|by name| DB
        end

        subgraph HOST_NET["host — no isolation, uses host network directly"]
            HC["container\nshares host IP"]
        end

        subgraph NONE_NET["none — fully isolated, no network"]
...
```mermaid
graph LR
    subgraph DEFAULT["Default Bridge — IP only, no DNS"]
        D1["Container A\n172.17.0.2"] -->|"ping 172.17.0.3 ✅\nping containerB ❌"| D2["Container B\n172.17.0.3"]
    end

    subgraph CUSTOM["Custom Bridge — DNS by name"]
        W["web"] -->|"ping db ✅\ncurl http://db:3306 ✅"| D["db"]
    end
```
